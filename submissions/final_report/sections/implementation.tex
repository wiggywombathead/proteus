...
void __attribute__((interrupt("ABORT"))) reset_handler(void);
    ...
    \subsubsection{Interrupt Requests}
        ...
        contains registers which may be communicated with via MMIO. These
        detail, for example, whether an IRQ is pending and control if the IRQ is
        enabled or disabled for a given interrupting device, and are given in
        the Technical Reference Manual \cite[pg.~112]{TRM}.
        ...
        the mapping between IRQs and devices; such a mapping is given below
        \cite{ARM_IRQs}:
        \begin{table}[h]
            \centering
            \begin{tabular}{|l|l|}
    ...
    \subsubsection{Handling an IRQ}
        ...
        routine. This is done by declaring the routine's type, along with a
        static array to hold each handler. The documentation also states
        \cite[pg.~109]{TRM} that, for each interrupt source, there is an enable
        bit, which is read/write, and a pending bit, which is read-only. Thus,
        it may not be cleared using the Interrupt Controller, but instead using
        the hardware device which initially triggered the interrupt. We
        therefore define the function type \code{interrupt\_clearer} which, for
        a given device, modifies the device's registers in order to deal with
        the clearing the interrupt. For example, with the System Timer, we set
        the \code{matched1} register to 1 (see Section \ref{sec:SysTimer}, and
        do not touch the Interrupt Controller pending bit itself.
